 ...
 * @param {string} nombre
 * @returns {Promise<{ codigo: string, validoHasta: string, mode: 'kv' | 'fs' | 'none' }>}
 */
export async function prepareCuponCode(prizeId, email, telefono, nombre = '') {
  ...
  /** Vercel sin KV: el FS no es persistente; igual generamos código y se manda el mail (sin ledger). */
  if (process.env.VERCEL === '1') {
    const codigo = `${prefix}-${randomBytes(5).toString('hex').toUpperCase()}`;
    console.warn(
      '[cupones] Vercel sin KV_REST_*: cupón efímero (email sí; sin registro en servidor). Enlazá Vercel KV para auditoría.'
    );
    return { codigo, validoHasta, mode: 'none' };
  }
